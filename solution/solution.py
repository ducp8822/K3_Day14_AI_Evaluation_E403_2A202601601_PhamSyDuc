...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        metrics are intentionally excluded from this core score.
        """
        return (
            self.faithfulness + self.relevance + self.completeness
        ) / 3.0
...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        rubric_text = json.dumps(rubric, ensure_ascii=False, default=str)
        prompt = (
            "You are an evaluation judge for a student-services assistant.\n"
            "Score the answer against every rubric criterion from 0 to 1.\n"
            "Return a JSON object with a `scores` object and an optional "
            "`reasoning` string. Do not score based on answer length alone.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_text}"
        )
        ...
        def get_score(criterion: str) -> float:
            value: Any = source.get(criterion)
            if value is None:
                criterion_lower = criterion.lower()
                for key, candidate in source.items():
                    if str(key).lower() == criterion_lower:
                        value = candidate
                        break
            try:
                score = float(value)
        ...
        scores = {criterion: get_score(criterion) for criterion in rubric}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        first_aliases = {
            "first", "first_score", "answer_a", "answer_a_score",
            "answer_1", "answer_1_score", "candidate_1", "position_1",
            "position_1_score", "response_1", "response_1_score",
        }
        second_aliases = {
            "second", "second_score", "answer_b", "answer_b_score",
            "answer_2", "answer_2_score", "candidate_2", "position_2",
            "position_2_score", "response_2", "response_2_score",
        }
        ...
        for item in scores_batch:
            ...
            normalized = {str(key).lower(): value for key, value in scores.items()}
            first = next(
                (normalized[key] for key in first_aliases if key in normalized),
                None,
            )
            second = next(
                (normalized[key] for key in second_aliases if key in normalized),
                None,
            )
            try:
                if first is not None and second is not None:
                    positional_pairs.append((float(first), float(second)))
            except (TypeError, ValueError):
                continue

        average = sum(all_scores) / len(all_scores) if all_scores else 0.0
        positional_bias = bool(positional_pairs) and all(
            first > second for first, second in positional_pairs
        )
        return {
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # The evaluator creates the score object; the benchmark owns the
            # original pair and must keep its metadata/retrieved chunks.
            result.qa_pair = pair
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        def retrieval_average(attribute: str) -> float | None:
            values = [
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        Compute average per answer-side metric and flag drops strictly greater
        than 0.05 versus the baseline.
        """
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        Build a Markdown table with one row per failure and a suggestion matched
        by position. Missing suggestions receive a safe fallback action.
        """
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = (
                ...
                else "Review the failure and validate the full evaluation pipeline"
            )
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        def add(suggestion: str) -> None:
            if suggestion not in suggestions:
                suggestions.append(suggestion)

        if "hallucination" in normalized_types or any(
            failure.faithfulness < 0.5 for failure in failures
        ):
            add(
                "Implement claim-level faithfulness checks and a guardrail that rejects unsupported claims."
            )
        if {"irrelevant", "off_topic"} & normalized_types or any(
            failure.relevance < 0.5 for failure in failures
        ):
            add(
                "Improve intent detection and prompt instructions so the answer directly addresses the question."
            )
        if "incomplete" in normalized_types or "low_completeness" in normalized_types or any(
            failure.completeness < 0.5 for failure in failures
        ):
            add(
                "Add answer checklists and few-shot examples covering required facts, conditions, exceptions, and deadlines."
            )

        # Keep the log actionable even when all failures share one category.
        add(
            "Review retrieval coverage and rerank relevant chunks so the generator receives the required evidence early."
        )
        add(
            "Add the failure pattern to the golden dataset and rerun the regression gate after each prompt or model change."
        )
        add(
            "Calibrate evaluation thresholds with human labels and route high-risk cases to manual review."
        )
